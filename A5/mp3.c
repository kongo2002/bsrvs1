...
void idTagFile(const char *fileName,char *comment)
{
    ...
    /* ID3-Tag Kommentar aendern, falls angegeben */
    if (comment != NULL)
    {
        ...
        if (fseek(datei, -31, SEEK_END) == -1)
        {
            fprintf(stderr, "Fehler beim Schreiben des ID3-Tags: %s\n", fileName);
            fclose(datei);
            return;
        }

        if ((rb = fwrite(comment, sizeof(char), 30, datei)) != 30)
        {
            fprintf(stderr, "Fehler beim Schreiben der Datei: %s\n", fileName);
            fclose(datei);
            return;
        }

#ifdef DEBUG
        fprintf(stderr, "%d Bytes erfolgreich geschrieben.\n", (int)rb);
#endif
